...
module.exports = async function handler(request, response) {
  ...
  if (!token || !chatId) {
    return response.status(503).json({ ok: false, error: 'Telegram delivery is not configured' });
  }
  ...
  try {
    ...
    if (!telegramResponse.ok || telegramResult.ok !== true) {
      return response.status(502).json({ ok: false, error: 'Telegram rejected the request' });
    }

    return response.status(200).json({ ok: true });
  } catch {
    return response.status(502).json({ ok: false, error: 'Telegram is temporarily unavailable' });
  }
};
